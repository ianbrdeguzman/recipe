...
vi.mock("@/lib/recipes/import/recipe-image-storage", () => ({
  storeImportedRecipeImage: vi.fn(),
}));
...
import { storeImportedRecipeImage } from "@/lib/recipes/import/recipe-image-storage";
...
describe("importRecipeFromUrl", () => {
  ...
  it("imports upstream on canonical cache miss, stores the imported image, then stores canonical and user rows", async () => {
    ...
    vi.mocked(storeImportedRecipeImage).mockResolvedValue(
      "imported/imported-1.webp" as never,
    );
    vi.mocked(createUserRecipeFromImportedRecipe).mockResolvedValue({
      id: "recipe-1",
      userId: "user-1",
    ...
    expect(createImportedRecipe).toHaveBeenCalledWith({
      importedRecipeId: expect.any(String),
      normalizedSourceUrl: "https://example.com/recipe",
      originalSourceUrl: "https://example.com/recipe/",
      input: expect.objectContaining({ title: "Pancakes" }),
      imageKey: "imported/imported-1.webp",
    });
    expect(storeImportedRecipeImage).toHaveBeenCalledWith({
      importedRecipeId: expect.any(String),
      imageUrl: "https://cdn.example.com/pancakes.jpg",
    });
    expect(result.id).toBe("recipe-1");
  ...
  it("continues importing when recipe image storage fails", async () => {
    const importer = vi.fn().mockResolvedValue({
      title: "Pancakes",
      description: null,
      imageUrl: "https://cdn.example.com/pancakes.jpg",
    ...
    vi.mocked(storeImportedRecipeImage).mockResolvedValue(null);
    vi.mocked(createUserRecipeFromImportedRecipe).mockResolvedValue({
      id: "recipe-1",
      userId: "user-1",
      importedRecipeId: "imported-1",
    ...
    expect(createUserRecipeFromImportedRecipe).toHaveBeenCalledWith({
      userId: "user-1",
      normalizedSourceUrl: "https://example.com/recipe",
      canonicalRecipe: expect.objectContaining({ imageKey: null }),
    });
